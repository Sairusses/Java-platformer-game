...
public class Eye extends Enemy{
	...
	private BufferedImage[][] eye = new BufferedImage[5][9];
	private Player player;
	
	
	private Rectangle attackRange;
    private int range = 70;
    public int eyeDamage = 10;
	public Eye(float x, float y, Player player) {
		super(x, y, 70, 70, 1);
		this.player = player;
		attackRange();
		loadImage();
	...
	private void loadImage() {
		...
		BufferedImage img = Load.GetSprite(Load.Eye_Idle);
		for(int x = 0; x < 8; x++) idle[x] = img.getSubimage(x * 150, 0, 150, 150);
		eye[0] = idle;
		
		run = new BufferedImage[8];
		img = Load.GetSprite(Load.Eye_Run);
		for(int x = 0; x < 8; x++) run[x] = img.getSubimage(x * 150, 0, 150, 150);
		eye[1] = run;
		
		attack = new BufferedImage[8];
		img = Load.GetSprite(Load.Eye_Attack);
		for(int x = 0; x < 8; x++) attack[x] = img.getSubimage(x * 150, 0, 150, 150);
		eye[2] = attack;
		
		damage = new BufferedImage[4];
		img = Load.GetSprite(Load.Eye_Damage);
		for(int x = 0; x < 4; x++) damage[x] = img.getSubimage(x * 150, 0, 150, 150);
		eye[3] = damage;
		
		dead = new BufferedImage[4];
		img = Load.GetSprite(Load.Eye_Death);
		for(int x = 0; x < 4; x++) dead[x] = img.getSubimage(x * 150, 0, 150, 150);
		eye[4] = dead;
	}
	public void render(Graphics g) {
		g.drawImage(eye[action][index],(int)x -130 + flipX, (int)y-129, 300 * flipW, 300, null);
//		drawHitbox(g);
//		drawAttackRange(g);
		drawHealthbar(g);
	}
